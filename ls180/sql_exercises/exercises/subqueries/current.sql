...
-- - each service that has 3 custoemrs 
...
-- - everybody has a bill 
-- - all pay in Times
-- - compute total gross income 
-- - gross income? 

-- per person all services count price 
-- person_id , all services, price of service 
...
-- - amount of expected income (cost more than 100)
-- - max income if every customer has all of a big ticket service
...
-- SELECT sum(services.price), count(customers.id)
--   FROM customers_services
--     FULL OUTER JOIN services
--       ON service_id = services.id
...
-- SELECT (count(DISTINCT customers.id) * 500.00) as sum
--   FROM customers
--   ;
